...
# Import your modules
from nfa import test_string_belongs_to_regex
from dfa import DFASimulator
from image_generator import AutomataImageGenerator
...
class RegexAutomataGUI:
    ...
    def display_min_dfa_table_text(self, pattern):
        ...
        try:
            ...
            if pattern == "aba":
                from minimized_dfa import display_aba_minimized_dfa
                output = self.capture_output(display_aba_minimized_dfa)
            elif pattern == "bb":
                from minimized_dfa import display_bb_minimized_dfa
                output = self.capture_output(display_bb_minimized_dfa)
            elif pattern == "c_only":
                from minimized_dfa import display_c_only_minimized_dfa
                output = self.capture_output(display_c_only_minimized_dfa)
            elif pattern == "ca":
                from minimized_dfa import display_ca_minimized_dfa
                output = self.capture_output(display_ca_minimized_dfa)
            elif pattern == "caa":
                from minimized_dfa import display_caa_minimized_dfa
                output = self.capture_output(display_caa_minimized_dfa)
            elif pattern == "caaa":
                from minimized_dfa import display_caaa_minimized_dfa
                output = self.capture_output(display_caaa_minimized_dfa)
            elif pattern == "c_kleene_star":
                from minimized_dfa import display_c_kleene_star_minimized_dfa
